...
# When using 'count' (in the module) to create the two records; it fails.
# So we refer to the index directly, which means we will have to add one more record for each region we enable (if any).
